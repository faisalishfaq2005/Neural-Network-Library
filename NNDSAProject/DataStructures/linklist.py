...
class LinkList:
    ...
    def insert_node(self, layer):
        if isinstance(layer, DenseLayer):
            ...
            if self.previous_output_size!= None:
                ...
                layer.weights = np.random.rand(layer.input_neurons, layer.neurons)
                layer.bias = np.random.rand(1, layer.neurons)
    ...
    def back_propogation(self,output_gradient):
        current_layer=self.head_pointer
        ...
        while current_layer.previous_pointer is not None:
            gradient,Layer_gradients=current_layer.backward
